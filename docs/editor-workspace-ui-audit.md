...
| Búsqueda de carpeta | Busca título, ruta, encabezados y contenido indexado; su cabecera identifica la carpeta autorizada y cada resultado muestra título, contexto de coincidencia y ruta relativa. | Falta QA visual de resultados extensos y de la jerarquía de esa información en una ventana estrecha. | El contexto se deriva solo del índice ya retenido: indica título, ruta, encabezado o un extracto breve de contenido, sin releer ni guardar más información de la bóveda. |
...
| Wikilinks | Soporta destino, alias y encabezado, con ambigüedad bloqueada; la fixture fija que una coincidencia corta ambigua no genera backlink. Un clic o Enter sobre un wikilink enfocado vuelve a resolverlo mediante VFS antes de abrirlo. El hub conserva un panel informativo con estado resuelto, ausente o ambiguo, sin releer disco. | Falta QA visual de textos largos y de los avisos cuando un destino se bloquea o resulta ambiguo. | Mantener el panel de diagnóstico inerte: la navegación explícita ocurre desde el enlace renderizado y no convierte el resultado indexado en una autoridad. |
